...
exports.insert_pedido = (req, res) =>{
  ...
  ped.save((err, result)=>{
    if(err) return res.status(500).json({code: 500, status: "Error", msg: "Error al guardar pedido"})
...
exports.insert_repartidor = (req, res) =>{
  ...
  Pedido.findByIdAndUpdate(idPed, {repartidor: idRep},(err, result)=>{
    if(err) return res.status(500).json({code: 500, status: "Error", msg: "Error al actulizar"})
...
exports.update_pedido = (req, res) =>{
  ...
  Pedido.findByIdAndUpdate(_id, {cantidad: cantidad, precio: precio}, (err, result)=>{
    if(err) return res.status(500).json({code: 500, status: "Error", msg: "Error al actulizar"})

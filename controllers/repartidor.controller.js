  ...
  usu
    ...
    .then((result) => {
      ...
      rep
        .save()
        .then((result) =>
          res.status(200).json({ code: 200, msg: "Repartidor insertado" })
        )
        .catch((err) =>
          res.status(404).json({ code: 404, status: "Error", msg: err + "" })
        );
    })
    .catch((err) =>
      res.status(404).json({ code: 404, status: "Error", msg: err + "" })
  ...
  Repartidor.findById(_id)
    .then((rep) => {
      Usuario.findByIdAndUpdate(rep.usuario, {
        ...
        res.status(200).json({ code: 200, msg: "Repartidor actualizado" })
      );
    })
    .catch((err) =>
      res.status(404).json({ code: 404, status: "Error", msg: err + "" })
...
exports.delete_repartidor = (req, res) => {
  ...
  Repartidor.findById(id)
    .then((rep) => {
      Usuario.findByIdAndDelete(rep.usuario).then((result) => {
        Repartidor.findByIdAndDelete(id).then((result) =>
          res.status(200).json({ code: 200, msg: "Repartidor eliminado" })
        );
      });
    })
    .catch((err) => res.status(404).json({ code: 404, msg: error + "" }));
